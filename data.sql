...
INSERT INTO animals (name, age, species_id)
 VALUES
     ('Pikachu', 5,
         CASE
             WHEN RIGHT('Pikachu', 3) = 'mon' THEN (SELECT id FROM species WHERE name = 'Digimon')
             ELSE (SELECT id FROM species WHERE name = 'Pokemon')
         END
     ),
     ('Charmander', 7,
         CASE
             WHEN RIGHT('Charmander', 3) = 'mon' THEN (SELECT id FROM species WHERE name = 'Digimon')
             ELSE (SELECT id FROM species WHERE name = 'Pokemon')
         END),
     ('Agumon', 6,
         CASE
             WHEN RIGHT('Agumon', 3) = 'mon' THEN (SELECT id FROM species WHERE name = 'Digimon')
             ELSE (SELECT id FROM species WHERE name = 'Pokemon')
         END),
     ('Squirtle', 8,
         CASE
             WHEN RIGHT('Squirtle', 3) = 'mon' THEN (SELECT id FROM species WHERE name = 'Digimon')
             ELSE (SELECT id FROM species WHERE name = 'Pokemon')
         END);
